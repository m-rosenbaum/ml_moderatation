...
test_that("gen_data produces correct number of taus below 0", {
  ...
  result <- gen_data(n_obs = n_obs, n_cols = 15, seed = seed, ate = ate)
  ...
  expect_equal(prop_negative, ate, tolerance = 0.005)
})
...
test_that("gen_data maintains statistical properties across different parameters", {
  ...
  test_cases <- list(
    list(n_obs = 5000,  seed=2, ate = 0),
    list(n_obs = 8000,  seed=3, ate = 0.1),
    list(n_obs = 12000, seed=4, ate = 0.25)
  )
  
  for(case in test_cases) {
    result <- do.call(gen_data, 
                     c(case, list(n_cols = 15)))
    
    tau_hat <- result$tau
    prop_negative <- mean(tau_hat < 0)
    
    # Test proportion of negative values
    expect_equal(prop_negative, mean(result$tau[result$treatment==1]), tolerance = 0.05)
...
test_that("gen_data handles edge cases", {
  ...
  result_min <- gen_data(n_obs = 1, n_cols = 11, seed=10, ate=0)
